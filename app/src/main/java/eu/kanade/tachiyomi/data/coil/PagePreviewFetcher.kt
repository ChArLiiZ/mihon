...
/**
 * Coil [Fetcher] 用於載入 [PagePreview] 的圖片。
 * 優先從 PagePreviewCache → DiskCache → 網路 載入。
 */
    ...
    class Factory(
        ...
        override fun create(data: PagePreview, options: Options, imageLoader: ImageLoader): Fetcher {
            return PagePreviewFetcher(
                ...
                sourceLazy = lazy {
                    val source = sourceManager.get(data.source) ?: return@lazy null
                    source as? PagePreviewSource
                },
